# Plan 07: Four-agent installer, managed state, and doctor
...
Implement installation, uninstall, configuration generation, backups, managed ownership, dry-run, and diagnostics for exactly Claude Code, Codex, Pi, and OpenCode. Actual discovery must be distinguished from file-copy success.

## Branch contract

- Stable branch: the branch detected by `mine init` (currently `master` for this repository).
- Integration branch: managed `dev`.
- Implementation branch: `plan/07-four-agent-installer-managed-state-and-doctor`.
...
`docs/plan/reports/07-four-agent-installer-managed-state-and-doctor-implementation.md`

## Downstream release

On independent acceptance, release: 08.
