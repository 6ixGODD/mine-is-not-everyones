...
Run link, marker, ownership, anchor, and document-size validation. Use:

- `mine design validate --format json` (returns `valid` and any warnings, including tracked `docs/design-backup-*` paths);
- `mine graph validate --format json` when a plan workspace is active;
- `mine design status --format json` for marker/identity confirmation.

A tracked `docs/design-backup-*` path is a release gate failure: backups are local recovery material and must not be committed.
