...
The machine source of truth is `docs/plan/execution-graph.toml`; `docs/plan/execution-graph.md` is generated by `mine graph render`. Never edit either graph file directly (`AGENTS.md` documents this rule). If `mine` is unavailable, complete architecture work but report graph initialization as blocked and provide the repository installation command.
